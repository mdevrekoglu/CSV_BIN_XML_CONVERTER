...
void convertCSVtoBIN(char *CSVFileName, char *BINFileName){
    ...
    while (fgets(line, sizeof(Person), CSVFile)) {
        ...
        // Read values from line and store them in person
        // Also get the result of sscanf to check whether there is an error or not
        int result = sscanf(line, "%[^,],%[^,],%c,%[^,],%[^,],%[^,],%[^,],%[^,],%[^,],%[^,],%u,%[^,\n]",
        ...
        // If any of the values of person is equal to ' ' then change it to NULL, ""
        if(strcmp(person.name, " ") == 0){
            strcpy(person.name, "");
        }
        if(strcmp(person.surname, " ") == 0){
            strcpy(person.surname, "");
        }
        if(strcmp(person.occupancy, " ") == 0){
            strcpy(person.occupancy, "");
        }
        if(strcmp(person.level_of_education, " ") == 0){
            strcpy(person.level_of_education, "");
        }
        if(strcmp(person.email, " ") == 0){
            strcpy(person.email, "");
        }
        if(strcmp(person.bank_account_number, " ") == 0){
            strcpy(person.bank_account_number, "");
        }
        if(strcmp(person.IBAN, " ") == 0){
            strcpy(person.IBAN, "");
        }
        if(strcmp(person.account_type, " ") == 0){
            strcpy(person.account_type, "");
        }
        if(strcmp(person.currency_unit, " ") == 0){
            strcpy(person.currency_unit, "");
        }
        if(strcmp(person.available_for_loan, " ") == 0){
            strcpy(person.available_for_loan, "");
        }

        fwrite(&person, sizeof(Person), 1, BINFile);
    ...
    // Print the result
    printf("CSV to BIN conversion is completed\n");
}
...
void converBINtoXML(char *BINFileName, char *XMLFileName){
    ...
    // In this part root node is created
    rootNode = xmlNewNode(NULL, BAD_CAST "records");
    // Set the root node
    xmlDocSetRootElement(doc, rootNode);

    // !! Attention !! due to document the root node's name should be the same as the file name
    // But on xsd file it can't be the same as the file name
    // Therefore xsd file can't validate the xml file
    // To solve this problem I changed the root node's name to records
    // But there is a code that makes the main root's name as file name
    // It is commented out

    // char *tempFileName = strtok(BINFileName, ".");
    // rootNode = xmlNewNode(NULL, BAD_CAST XMLFileName);
